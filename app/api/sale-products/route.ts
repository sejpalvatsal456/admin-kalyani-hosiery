...
import { SaleProduct } from "@/lib/models";
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (id) {
      const prod = await SaleProduct.findById(id)
        .populate("brandId")
        .populate("categoryId")
    ...
    const totalCount = await SaleProduct.countDocuments(query);
    ...
    // Execute aggregation
    const products = await SaleProduct.aggregate(pipeline);
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const created = await SaleProduct.create({
      ...data,
...
export const PATCH = async (req: NextRequest) => {
  try {
    ...
    const prod = await SaleProduct.findById(_id);
...
export const DELETE = async (req: NextRequest) => {
  try {
    ...
    await SaleProduct.deleteOne({ _id: id });
