...
export default function AddSaleProductPage() {
  ...
  // 🔥 Submit sale product with /api/sale-products endpoint
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        alert(data.msg || "Failed to create sale product");
        return;
      }

      // 🔥 Redirect to sale product variant editor
      router.push(`/editSaleVarient/${data._id}`);
    } catch (err) {
      console.error(err);
      ...
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_35px_60px_-35px_rgba(15,23,42,0.35)]">
        ...
        <div className="border-b border-slate-200 px-6 py-8 sm:px-10">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Create a new sale product with details and descriptions.
          </p>
        </div>
